...
# This runs *.sh, *.sql, *.sql.zip, *.sql.gz files and traverses in sub directories
...
if healthcheck.sh "${ORACLE_SID}" "container status ignore"; then
  ...
  if [ -z "${DATABASE_ALREADY_EXISTS:-}" ]; then
    ...
    if [ -n "${APP_USER:-}" ]; then
        ...
        for new_app_user in ${APP_USER}; do
            echo "CONTAINER: Creating app user '${new_app_user}' for default pluggable database."
            # Create app user for default database
            createAppUser "${new_app_user}" "${APP_USER_PASSWORD}"
            echo "CONTAINER: DONE: Creating app user '${new_app_user}' for default pluggable database."
            ...
            if [ -n "${ORACLE_DATABASE:-}" ]; then
                ...
                createAppUser "${new_app_user}" "${APP_USER_PASSWORD}" "${new_pdb}"
  ...
  if [[ $(cat /etc/oci-image-version) != "23.26.0" &&
        $(cat /etc/oci-image-version) != "23.9" ]]; then
    echo ""
    echo "################################################"
