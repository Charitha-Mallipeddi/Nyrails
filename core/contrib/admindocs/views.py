...
class CustomModelDetailView(ModelDetailView):
    def get_context_data(self, **kwargs):  # noqa: C901, PLR0912, PLR0915
        ...
        title, body, metadata = utils.parse_docstring(model.__doc__)  # pyright: ignore[reportArgumentType] # noqa: RUF059
        ...
        for field in opts.fields:
            ...
            if isinstance(field, models.ForeignKey):
                ...
                verbose = ", ".join(field.field_names)
            elif isinstance(field, models.ForeignObject):
                data_type = field.remote_field.model.__name__
                app_label = field.remote_field.model._meta.app_label
                verbose = utils.parse_rst(
                    (
                        _(
                            "the related `%(app_label)s.%(data_type)s` foreign object "
                            "\n\nfrom %(from_fields)s \n\nto %(to_fields)s"
                        )
                        % {
                            "app_label": app_label,
                            "data_type": data_type,
                            "from_fields": field.from_fields,
        ...
        for func_name, func in model.__dict__.items():
            if inspect.isfunction(func) or isinstance(
                ...
                # arguments'.
                if isinstance(func, (cached_property, property)) or (
        ...
        return {
            **kwargs,
            "name": opts.label,
            "summary": utils.strip_p_tags(title),
            "description": body,
            "fields": fields,
            "methods": methods,
        }
